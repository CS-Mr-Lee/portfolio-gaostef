...
public class MenuScreen implements Screen {
	 ...
	 * 
	 * @param MyGdxGame game         - the game name but I didn't know how to rename it so it is the
	 * 								   default
	 */
	...
	public void render(float delta) {
		...
		if (Gdx.input.getX()< x + EXIT_BUTTON_WIDTH && Gdx.input.getX() > x && 400 - Gdx.input.getY() < EXIT_BUTTON_Y + EXIT_BUTTON_HEIGHT && 400 - Gdx.input.getY() > EXIT_BUTTON_Y) {
		...
		//Play game button
		//setting the parameters 
		if (Gdx.input.getX()< x + PLAY_BUTTON_WIDTH && Gdx.input.getX() > x && 400 - Gdx.input.getY() < PLAY_BUTTON_Y + PLAY_BUTTON_HEIGHT && 400 - Gdx.input.getY() > PLAY_BUTTON_Y) {
